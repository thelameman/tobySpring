...
$(document).on('click','#click',function(event){
	...
	window.open('agencyPicPop?brokerId='+brokerId,'pop','resizable=no scrollbars=yes top=500 left=500 width=700 height=800');
})
...
var	officeName = null;
		$.ajax({
			...
			success: function(response){
				...
				$.each(list , function(index,item){
					var tag = '<td>'+item.brokerId+'</td><td>'+item.name+'</td><td><a href="" id="click" broker="'+item.brokerId+'">' +item.officeName+ '</a></td><td>' +item.address+'</td><td>'+item.officeTel+'</td>';
					$('<tr></tr>').html(tag).insertAfter('#brList');
					var confirm = $('<input/>').attr('type','button').attr('id','con').attr('class','btn pull-center btn-default').attr('bId',item.brokerId).val('승인');
				$('td:eq(4)').after(confirm.clone()); 
				});
			}
		});
		...
		$(document).on('click','#con' ,function(){
			var brokerId = +$(this).attr('bId');
			$.ajax({
				url:'approveBroker',
				method:'get',
				data: 'brokerId='+$(this).attr('bId'),
				dataType:'json',
				success:function(response){
					alert('승인완료');
				}
						...
						<div class="col-md-12 col-sm-12">
							...
							<article id="post-196"
								...
								<section id="content" class="content">
									<header class="">
										<h1>중개사 승인 목록</h1>
										 <a href="approvePropertyList"
											style="position: fixed; top: 110px; right: 600px">매물 승인
											페이지</a>  <a href="QnAListView"
																			...
																			<table class="table table-striped">
																				<thead>
																					<tr id="brList"
																						...
																						<th width="10%;" style="text-align: center">중개자
																							ID</th>
																						<th width="10%;" style="text-align: center">대표자
																							명</th>
																						<th width="20%;" style="text-align: center">중개소
																							명</th>
																						<th width="30%;" style="text-align: center">중개소
																							주소</th>
																						<th width="20%;" style="text-align: center">중개소
																							전화번호</th>
