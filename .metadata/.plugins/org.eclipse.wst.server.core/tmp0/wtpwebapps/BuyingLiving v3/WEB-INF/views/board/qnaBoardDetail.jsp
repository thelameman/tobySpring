...
<script>
	$(function() {
		...
		$('#qnaBoardView').on('click', function() {
			//alert("freeBoardView");
			window.location.href = "qnaBoardView?currentPage=1";
		});
		...
		$('#updateBoard').on('click', function() {
			...
			if(replyCount>0){
				alert('관리자의 댓글이 있을 경우 수정 할 수 없습니다.');
				return false;
			}
		...
		$('#deleteBoard').on('click', function() {
			...
			var replyCount = ${board.replyCount};
			if(replyCount>0){
				alert('관리자의 댓글이 있을 경우 삭제 할 수 없습니다.');
				return false;
			}
			
			window.location.href = "qnaBoardDelete?boardNo="+${board.boardNo};
		});
		...
		$('.del').on('click', function(){
			...
			if(delConfirm){
				...
				window.location.href = "qnaDeleteReply?replyNo="+replyNo+"&currentBoardNo="+currentBoardNo;
			}
		});
																...
																<c:if test="${not empty board }">
																	<div class="table-responsive">
																		<table class="table table-striped">
																			...
																			<tr>
																				<th>내용</th>
																				<td colspan="7"><textarea id="boardText"
																						disabled cols="120%;" rows="10%"
																						style="padding: 10px;" class="form-control">${board.boardText }</textarea>
																				</td>
																			</tr>
...
$(document).ready(function() {
    ...
    $('span.tag.price').on('click', function() {
    	if($(this).html()=='Download'){
    		...
    		var f =  $(this).attr('f');
			var p =  $(this).attr('p');
			window.location.href="qnaDownload?f="+f+"&p="+p;
    	}
    });
});
</script>
																			...
																			<span> <c:choose>
																					<c:when test="${loginId  == 'admin' }">
																						<a class="btn pull-right btn-default"
																							style="margin-left: 15px;" href="QnAListView"
																							style="position: fixed; top: 180px; right: 400px">QnA
																							미답변 페이지</a>
																					</c:when>
																				</c:choose> 
																					 
																						
																						<input type="button" id="qnaBoardView" value="목록"
																...
																<hr>
																<br>
																<form action="insertQnaReply" method="post">
