...
<link rel="canonical"
	href="http://themes.fruitfulcode.com/zoner/template-password-protected/" />
<link rel='shortlink'
	...
	var par = "sendId="+sendId+"&receiveId="+receiveId+"&msgText="+msgText;
		...
		$.ajax({
			...
			success: function(response){
				...
				$('[name=receiver]:last').append('<figure class="icon"><i class="fa fa-archive"></i></figure>');
			...
			<div class="row">
				<div class="col-md-9 col-sm-9">
					<article id="post-1168"
						...
						<header>
							<h1 class="entry-title">${receiveCustomer.name}${receiveBroker.name}님과의
								쪽지</h1>
						</header>

						<a href="msgBoxView?sendId=${param.sendId}"
							style="position: absolute; top: 60px; right: 30px">메시지함으로</a>
						<div id="msgList">
							<c:forEach items="${wholeList}" var="m">
								<c:if test="${param.sendId == m.sendId }">
								...
								<aside class="description">
									<header>
										<h3 style="font-size: 10pt;">${m.msgRegDate}</h3>
									</header>
									<p style="font-size: 10pt;">${m.msgText}</p>
								</aside>
						</div>
						<!-- /.feature-box -->
				</div>
			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					<input type="hidden" id="receiveId" name="receiveId"
						value="${param.receiveId}"> <input type="hidden"
						id="sendId" name="sendId" value="${param.sendId}"> <input
