...
$(function(){
		...
		success: function(response){
			...
			$.each(list , function(index,item){
				...
				var tag = "<th>"+item.propertyNo+"</th><th><a href='javascript:msgbox("+item.propertyNo+")' pNo="+item.propertyNo+" pid='"+item.sellerId+"' >"+item.sellerId+"</a></th><th>"+item.propertyType +"</th><th><a href='javascript:detailView("+item.propertyNo+")'>"+item.address+"</a></th><th>"+item.price+"</th>"+"</th><th></th>";
				$('<tr></tr>').html(tag).attr('id','tr'+item.propertyNo).insertAfter('#prList');
				var confirm = $('<input/>').attr('type','button').attr('id','con-'+item.propertyNo).attr('class','btn pull-center btn-default').attr('pNo',item.propertyNo).val('승인');
				if(item.propertyType == '월세'){
					$('th:eq(11)').append('/'+item.monthfee);
				}
			$('th:eq(11)').append(confirm.clone()); 
			$('#con-'+item.propertyNo).on('click',function(){
				$.ajax({
					url:'approveProperty',
					method:'get',
					data: 'propertyNo='+item.propertyNo,
					dataType:'text',
					success:function(response){
						$('#tr'+item.propertyNo).remove();
						alert('승인완료');
					}
...
function msgbox (pno){
	seller = $('[pNo='+pno+']').attr('pid');
	
	var url = "selectOneMsgBox?sendId=${loginId}&receiveId="+seller;
...
})('//themes.fruitfulcode.com/zoner/?wordfence_logHuman=1&hid=524FA64CB247C53CCA01061882E71B5C');
</script>
<script type="text/javascript">
			var _gaq = _gaq || [];
						...
						<div class="col-md-12 col-sm-12">
							...
							<article id="post-196"
								...
								<section id="content" class="content">
									<header class="">
										<h1>매물 승인 목록</h1>
											 <a href="approveBrokerList"
											style="position:fixed; top: 110px; right: 600px">중개사 승인
											페이지로</a>  <a href="QnAListView"
											style="position:fixed; top: 110px; right: 400px">QnA 미답변 페이지</a> 
											<br><br>
									</header>
																			...
																			<table class="table table-striped">
																				<thead>
																					<tr id="prList"
																						...
																						<th width="10%;" style="text-align: center">매물 번호</th>
																						<th width="10%;" style="text-align: center">판매자 ID</th>
																						<th width="20%;" style="text-align: center">매물 형태</th>
																						<th width="30%;" style="text-align: center">매물 주소</th>
																						<th width="20%;" style="text-align: center">(전/월세) 가</th>
																						<th width="10%;" style="text-align: center">승인여부</th>
																					</tr>
																				</thead>
																				<tbody>
